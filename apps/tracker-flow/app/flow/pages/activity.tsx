 ...
 * means resolving names is this page's job.
 *
 ...
 *
 * Identical to the other two builds, deliberately. The audit trail has no live
 * event to subscribe to — `Auditable` writes rows, it does not broadcast — so a
 * Flow version that updated itself would need a broadcast this app does not
 * emit. Inventing one here to make the page look reactive would be a claim about
 * the framework that the framework is not making.
 */
export class ActivityPage extends Component {
  ...
  async render(): Promise<HtmlNode> {
    ...
    const actorIds = [
      ...new Set(entries.map((e) => e.actorId).filter((id): id is number => !!id)),
    ];
    const issueIds = [
      ...new Set(
        entries
    ...
    return (
      <div class="max-w-3xl space-y-6">
        <div>
          <h1 class="text-xl font-semibold tracking-tight">{__("Activity")}</h1>
          <p class="mt-1 text-sm text-muted-foreground">{__("Every change, most recent first.")}</p>
        </div>

        {entries.length === 0 ? (
          <div class={`${CARD} p-10 text-center`}>
            <h2 class="text-sm font-medium text-foreground">{__("Nothing has changed yet")}</h2>
          ...
          <ul class="space-y-2">
            {entries.map((entry) => {
              ...
              return (
                <li key={String(entry.id)} class={`${CARD} p-4`}>
                  <p class="text-sm">
                    ...
                    {subject?.title && subject.projectSlug && entry.auditableId ? (
                      <Link
                        href={`/projects/${subject.projectSlug}/issues/${String(entry.auditableId)}`}
                        down
