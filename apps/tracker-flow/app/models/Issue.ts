 ...
 */
@table("issues")
export class Issue extends Model.using(Auditable) {
  static override fillable = ["title", "body", "status", "priority", "assigneeId", "dueAt"];

  /**
   * What the trail records — feature 11.
   *
   * An allowlist rather than a denylist, because the interesting question about
   * an issue is who moved it and who it landed on. `body` is deliberately out:
   * storing every revision of a long description in the audit table doubles the
   * database to answer a question the activity feed never asks.
   */
  protected static auditOnly = ["title", "status", "priority", "assigneeId"];
   ...
   *
   * **Every branch ends on `id`, and that is not decoration.** None of these
   * columns is unique: two issues created in the same millisecond, or sharing a
   * priority, or — for `title` — named the same thing, are tied, and a tied
   * `ORDER BY` returns whatever the engine finds convenient. That is invisible
   * on one screen of results and corrupting under `LIMIT`/`OFFSET`: the tie can
   * resolve differently for each page, so a row shows up on both page one and
   * page two while another is never returned at all. Seeding a hundred issues in
   * a loop makes it reproducible — 118 rows landed on 109 distinct timestamps.
   *
   * The tiebreaker runs the same direction as the sort above it, so it reads as
   * a continuation of that order rather than a reversal inside each tie group.
   */
