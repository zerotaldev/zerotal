...
import { createStreamStore, runWithStreaming, streamChunk } from "./streaming.ts";
 ...
 *
 * @internal Exported so the document-assembly path — section resolution and the
 * streamed response — can be driven directly in tests, without standing up a
 * router and a server to reach it.
 */
export function _makeFlowHandler(path: string, PageClass: PageClassWithMeta) {
  return class FlowPageHandler {
    async handle(ctx: HttpContext): Promise<void> {
      ...
      // stack leak.
      // Both stores are per-request and must stay active for the whole render:
      // sections are published during the page render and read after the layout
      // one, and a streamed child is queued mid-render and drained afterwards.
      const sections = createSectionStore();
      const streams = createStreamStore();

      try {
        await runWithSections(sections, () =>
          runWithStreaming(streams, async () => {
...
<body>
  ...
  ${runtimeTag}
            ...
            const document = resolveSections(html, sections);

            if (streams.pending.length === 0) {
              http.html(document);
              return;
            }

            // Streamed children: flush the document up to </body>, then append each
            // child's markup as it finishes, then close. Constructed here, inside the
            // request's async context, so the renders below still see the same
            // HttpContext and section store.
            const closing = document.lastIndexOf("</body>");
            const shell = closing === -1 ? document : document.slice(0, closing);
            const tail = closing === -1 ? "" : document.slice(closing);
            const queued = [...streams.pending];
            ...
            const body = new ReadableStream<Uint8Array>({
              async start(controller) {
                ...
                for (const entry of queued) {
                  let markup: string;
                  try {
                    markup = await entry.render();
                  } catch (error) {
                    // The shell is already on the wire, so there is no failing the
                    // response — the placeholder is replaced with a notice instead.
                    console.error(
                      `[Flow] Streamed child ${entry.childId} failed to render:`,
                      error,
                    );
                    markup =
                      '<div data-flow-stream-error role="alert" class="rounded border ' +
                      'border-red-300 bg-red-50 p-3 text-sm text-red-700">' +
                      "Something went wrong loading this section.</div>";
                  }
                  controller.enqueue(encoder.encode(streamChunk(entry.childId, markup)));
                }
            ...
            http.response = new Response(body, {
              headers: {
                "Content-Type": "text/html; charset=utf-8",
                // Proxies that buffer defeat streaming; this asks nginx not to.
