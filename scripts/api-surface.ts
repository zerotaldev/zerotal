...
function renderPackage(program: ts.Program, checker: ts.TypeChecker, pkg: PackageEntry): string {
  ...
  return normalize(
    `# ${pkg.name} — public API surface\n\n` +
      ...
      // This file is a change-detection snapshot, not a guide: every export at one
      // altitude, alphabetically, with no indication of which twenty you actually
      // reach for. Someone who lands here first can read the whole ORM surface and
      // still conclude the query builder is thin, because `whereIn` sits between
      // `whereHas` and `whereJson` among two hundred siblings. The pointer costs a
      // line and names what this file is for.
